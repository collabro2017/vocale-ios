...
- (void) processResponseData:(NSData *)data {
    ...
    NSRange range = [jsonString rangeOfString:@"{" options:NSCaseInsensitiveSearch range:NSMakeRange(0, 20)];
    if (range.location != 0 && range.location != NSNotFound) {
        jsonString = [jsonString substringFromIndex:range.location];
        
        range = [jsonString rangeOfString:@"}" options:NSBackwardsSearch range:NSMakeRange(0, jsonString.length)];
        jsonString = [jsonString substringWithRange:NSMakeRange(0, range.location + 1)];
    }
	
	NSError *error = nil;
	id jsonData = [NSJSONSerialization JSONObjectWithData:[jsonString dataUsingEncoding:NSUTF8StringEncoding]
