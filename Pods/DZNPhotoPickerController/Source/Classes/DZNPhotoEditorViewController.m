...
- (void)prepareSubviews
{
    if (!_imageView.image)
    {
        ...
        [_imageView sd_setImageWithPreviousCachedImageWithURL:_photoMetadata.sourceURL placeholderImage:nil options:SDWebImageCacheMemoryOnly|SDWebImageProgressiveDownload|SDWebImageRetryFailed progress:NULL completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
            if (!error) {
                _button.enabled = YES;
            }
            else {
