...
- (void)configureBubbleViewForVideoContent
{
   ...
   // self.bubbleViewWidthConstraint.constant = [[self class] cellSizeForImageMessage:self.message].width;
    ...
    displayingImage = [UIImage imageNamed:@"VoiceNoteIcon"];
    ...
    [self.bubbleView updateWithVideoThumbnail:displayingImage width:displayingImage.size.width];
}
/*
- (void)configureBubbleViewForAudioContent
{
    self.accessibilityLabel = @"Audio/MP4";
    self.bubbleViewWidthConstraint.constant = [[self class] cellSizeForImageMessage:self.message].width;
    
    LYRMessagePart *fullResAudioPart = ATLMessagePartForMIMEType(self.message, @"audio/m4a");
    if (fullResAudioPart && ((fullResAudioPart.transferStatus == LYRContentTransferAwaitingUpload) ||
                             (fullResAudioPart.transferStatus == LYRContentTransferUploading))) {
        // Set self for delegation, if full resolution message part
        // hasn't been uploaded yet, or is still uploading.
        LYRProgress *progress = fullResAudioPart.progress;
        [progress setDelegate:self];
        self.progress = progress;
        [self.bubbleView updateProgressIndicatorWithProgress:progress.fractionCompleted visible:YES animated:NO];
    }
    ...
    UIImage *displayingImage = [UIImage imageNamed:@"VoiceNoteIcon"];
    ...
    [self.bubbleView updateWithAudioThumbnail:displayingImage width:size.width];
}*/
...
+ (CGFloat)cellHeightForMessage:(LYRMessage *)message inView:(UIView *)view
{
    ...
    if ([part.MIMEType isEqualToString:ATLMIMETypeVideoMP4]) {
        height = ATLMessageBubbleMapHeight/2;
    }
