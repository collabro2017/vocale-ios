...
#define kClockLabelFont [UIFont fontWithName:@"Raleway-Regular" size:valForScreen(16,19)]
#define kDayLabelFont [UIFont fontWithName:@"Raleway-Regular" size:valForScreen(16,19)]
...
#define kDefDarkColor           color(0,0,0,1)
...
- (void)panGestureRecognized:(UIPanGestureRecognizer *)sender
{
    ...
    NSInteger minutesPerInterval = 30;
    CGFloat angleInterval = 15;
    NSLog(@"%f",vel);
    if(vel > 1400){
        minutesPerInterval = 90;
        angleInterval = 10;
    }
...
-(void)updateForDate:(NSDate*)date{
    ...
    self.clockLabel.text = [NSString stringWithFormat:@"%@: %@", dayString, timeString];
    self.dayLabel.text = @"Set the time for your event:";
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
       ...
       // [self.confirmButton setImage:[UIImage imageNamed:@"checkmark_icon_white"] forState:UIControlStateHighlighted];
        UILabel *label = [[UILabel alloc] initWithFrame:self.confirmButton.frame];
        label.text = @"Set";
        label.textColor = [UIColor whiteColor];
        label.textAlignment = NSTextAlignmentCenter;
        label.font = [UIFont fontWithName:@"Raleway-Regular" size:26];
        ...
        self.dayLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 120, self.bounds.size.width, 120)];
        ...
        self.clockLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 120, self.bounds.size.width, 120)];
        self.clockLabel.backgroundColor = [UIColor clearColor];
        self.clockLabel.textColor = [UIColor lightTextColor]; //self.foregroundColor;
        self.clockLabel.font = kClockLabelFont;
        self.clockLabel.numberOfLines = 2;
...
-(void)layoutSubviews{
    ...
    self.dayLabel.frame = CGRectMake(0, spacing+heightForDay, self.bounds.size.width, heightForDay);
    self.clockLabel.frame = CGRectMake(0, spacing+heightForDay+heightForDay, self.bounds.size.width, heightForTime);
    
    self._distanceForIcons = self.timeSlider.frame.size.height/2 + spacing*0.5 + iconHeigt/2;
}
