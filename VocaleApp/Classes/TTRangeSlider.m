...
const float HANDLE_DIAMETER = 24;
const float TEXT_HEIGHT = 18;
...
static const CGFloat kLabelsFontSize = 15.0f;
...
- (void)initialiseControl {
    ...
    self.sliderLine.backgroundColor = [UIColor whiteColor].CGColor;
    [self.layer addSublayer:self.sliderLine];

    //draw the minimum slider handle
    self.leftHandle = [CALayer layer];
    self.leftHandle.cornerRadius = 12.0f;
    self.leftHandle.borderColor = [UIColor whiteColor].CGColor;
    self.leftHandle.borderWidth = 3;
    ...
    self.rightHandle.cornerRadius = 12.0f;
    self.rightHandle.borderColor = [UIColor whiteColor].CGColor;
    self.rightHandle.borderWidth = 3;
...
- (void)layoutSubviews {
    ...
    self.sliderLine.frame = CGRectMake(lineLeftSide.x, lineLeftSide.y, lineRightSide.x-lineLeftSide.x, 5);
...
- (void)updateLabelPositions {
    ...
    CGPoint newMinLabelCenter = CGPointMake(leftHandleCentre.x, self.leftHandle.frame.origin.y - padding);

    CGPoint rightHandleCentre = [self getCentreOfRect:self.rightHandle.frame];
    CGPoint newMaxLabelCenter = CGPointMake(rightHandleCentre.x, self.rightHandle.frame.origin.y - padding);
